...
namespace Wis.Toolkit
{
    public sealed class ChineseSpell
    {
        ...
        #region ChineseSpells
        private static readonly string[][] chineseSpells =
         new string[][]
   {
    ...
    new string[]{"Zuo","做作坐左座昨凿琢撮佐笮酢唑祚胙怍阼柞乍侳咗岝岞挫捽柮椊砟秨稓筰糳繓苲莋葃葄蓙袏諎醋鈼鑿飵嘬阝"}
        ...
        /// </summary>
        /// <param name="str">简体汉字字符。</param>
        /// <returns>返回汉语拼音。</returns>
        public static string ToSpell(string str)
        {
            if (str == null) return string.Empty;
            Encoding encoding = Encoding.GetEncoding("GB2312");
            if (encoding == null) return string.Empty; // 没有找到编码集GB2312
            ...
            {
                bytes = encoding.GetBytes(chars[index].ToString());
                if (bytes.Length == 1)
                {
                    sb.Append(chars[index]); // 处理 0 - 9 数字的拼音

                    continue;
                }
                encodingIndex = bytes[0] - 0xA0;
                if (0x10 <= encodingIndex && encodingIndex <= 0x57) // 是gb2312汉字
                {
                    bool hasChineseSpell = false;
                    for (int spellIndex = 0; spellIndex < chineseSpells.Length; spellIndex++)
                    {
                        if (chineseSpells[spellIndex][1].IndexOf(chars[index]) != -1)
                        {
                            sb.Append(chineseSpells[spellIndex][0]);
                            hasChineseSpell = true;
                            break;
                        }
                    }
                    if (!hasChineseSpell)
                        sb.Append(chars[index]); // TODO:转换拼音不应该输出简体汉字本身

                }
                else
                    sb.Append(chars[index]); // TODO:转换拼音不应该输出繁体汉字本身
        ...
        /// <summary>
        /// 阿拉伯数字转换为汉语大写数字的拼音首字母。

        /// </summary>
        /// <param name="str">阿拉伯数字。</param>
        /// <returns>返回汉语大写数字的拼音首字母。</returns>
        public static char ToDigitSpell(char c)
        {
            if ((int)c < '0' || (int)c > '9') return c;

            // 零ling 壹yi 贰er 叁san 肆si 伍wu 陆liu 柒qi 捌ba 玖jiu
            switch (c)
            {
                case '0':
                    return 'L';
        ...
        #region InitialChineseSpells
        private static string[] initialChineseSpells = 
            {
        ...
        /// <param name="str">简体汉字字符。</param>
        /// <returns>返回汉语拼音。</returns>
        public static string ToInitialSpell(string str)
        {
            ...
            if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) // 若是字母则直接输出 
                sb.Append(char.ToUpper(c));
            else if ((int)c >= '0' && (int)c <= '9') // 数字
            {
                sb.Append(ToDigitSpell(c));
            }
            else if (c == '.') // 小圆点
            ...
            else if ((int)c >= 19968 && (int)c <= 40869)
            {
                // 若字符Unicode编码在编码范围则 查汉字列表进行转换输出 
                foreach (string initialChineseSpell in initialChineseSpells)
                {
                    if (initialChineseSpell.IndexOf(c) > 0)
                    {
                        sb.Append(initialChineseSpell[0]);
                        break;
                    }
                }
            }
