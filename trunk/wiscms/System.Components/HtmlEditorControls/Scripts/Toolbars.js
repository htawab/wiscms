...
function HtmlEditor_AttachFile(designer, designerView, dialogsPath) {
	...
	var script = dialogsPath + 'AttachFile.aspx'; // TODO:允许上传的文件类型
...
function HtmlEditor_InsertFlash(designer, designerView, dialogsPath) {
	...
	var script = dialogsPath + 'InsertFlash.htm';
	showModalDialog(script,designer,'dialogWidth:350px; dialogHeight:200px;help:0;status:0;resizeable:1;');
}
...
function HtmlEditor_FontSizes(designer,designerView,name,value) {
	if (designerView != 'DesignView') return;
	designer.focus();
	designer.document.execCommand('fontsize','',value);
}
