...
ImageCropper.prototype = {
  ...
  initialize: function(container, handle, url, options) {
	...
	//设置预览对象
	this.Preview = $(this.options.Preview);// 预览对象
	if(this.Preview){
	...
	//设置样式
	this._Container.style.position = "absolute";// "relative";
  ...
  SetSize: function() {
	...
	var w = this._tempImg.width; // s.Width
	var h = this._tempImg.height;// s.Height
	// 设置容器
	...
	//设置缩放范围
	if(this.Resize){ this._resize.mxRight = w; this._resize.mxBottom = h; }
  },
  //获取当前样式
  GetPos: function() {
  ...
  Close: function() {
	...
	// this._layBase = this._Container.appendChild(document.createElement("img")); // 底层
	if(!!this._layBase) this._Container.removeChild(this._layBase);
	// this._layCropper = this._Container.appendChild(document.createElement("img")); // 切割层
	if(!!this._layCropper) this._Container.removeChild(this._layCropper);
	if(!!this._tempImg) this._tempImg == null;
	// this._Dragger = this._Handle.appendChild(document.createElement("div"));
	if(!!this._drag._Dragger && !!this._drag._Handle) this._drag._Handle.removeChild(this._drag._Dragger);
  }  
}
