...
var imgUpLoadBox = {
    ...
    init: function(title, message, bSlide){
        ...
        //window.onload = function(){
           imgUpLoadBox.createDiv();
           imgUpLoadBox.setEvent();
        //}
    }, 
    ...
    createDiv: function(obj){
       ...
       shadow.style.height = document.body.offsetHeight + 'px';
		   ...
		   windowContent.innerHTML='<iframe id="oIframe" name="oIframe" src="../images/HtmlEditor/Dialogs/InsertImg/InsertImg.htm" frameborder="0" scrolling="no"></iframe>'	   
		   //widowContent over
	   	    	       ...
	   	    	       if(i>100 || j>1){
			                  ...
			                  if(window.opera){
			                       this.oUpLoadWindow.style.opacity = 1;
	   	    	   	             ...
	   	    	   	             imgUpLoadBox.oUpLoadWindow.style.filter ='';
	   	    	            }
	   	    	            return false;
	   	    	        }
			              if(window.opera){
			                  imgUpLoadBox.oUpLoadWindow.style.opacity = j;
			                  j += 0.1;
			              }
			              else{	
	   	                  imgUpLoadBox.oUpLoadWindow.style.filter = 'alpha(opacity='+i+')';
	   	    	            i += 10;
	   	              }    
	   	       }
	   	       else{
	   	    	        if(j>1){
	   	    	             if(tt) tt=window.clearInterval(tt);
	   	    	             imgUpLoadBox.oUpLoadWindow.style.opacity=1;
	   	    	             return false;
	   	    	        }
			              imgUpLoadBox.oUpLoadWindow.style.opacity = j;
			              j += 0.1;
...
window.onresize=function(){
	   if(imgUpLoadBox.oUpLoadWindow){
	   	    var yScroll = imgUpLoadBox.getPageScroll()[1];
	   	    var pageHeight = imgUpLoadBox.getPageHeight();
	   	    var marginTop = yScroll+(pageHeight-180)/2;
	        imgUpLoadBox.marginTop= marginTop;
	        imgUpLoadBox.fixPos();
	   }
}
