...
#else
                    appSetting = ConfigurationManager.AppSettings["Anthem.IncludePageScripts"];
#endif
...
#else
            contentEncoding = ConfigurationManager.AppSettings["Anthem.ResponseEncoding"];
            contentType = ConfigurationManager.AppSettings["Anthem.ResponseType"];
#endif
            ...
            if (!CheckIfRedirectedToLoginPage() && IsCallBack)
            {
                ...
                if (_updatePage)
                {
                    ...
                    if (_updateValidationScripts)
                    {
                        ...
                        IDictionary dictionary = (IDictionary)HttpContext.Current.GetSection("system.web/webControls");
                        if (dictionary != null)
                        {
...
//]]>
...
#else
            page.ClientScript.RegisterClientScriptBlock(this.GetType(), typeof(Manager).FullName, pageScript);
#endif
            ...
            page.ClientScript.RegisterClientScriptBlock(this.GetType(), "Anthem.js", 
...
#endif
#else
            string baseUri = System.Configuration.ConfigurationManager.AppSettings["Anthem.BaseUri"];
            if (baseUri != null && baseUri.Length > 0)
            {
                if (baseUri.StartsWith("~"))
                    baseUri = page.ResolveUrl(baseUri);
                string scriptUri = System.IO.Path.Combine(baseUri, "Anthem.js");
                page.ClientScript.RegisterClientScriptBlock(this.GetType(), "Anthem.js", string.Format("<script type='text/javascript' src='{0}'></script>", scriptUri));
            }
            else
            {
                Stream stream = typeof(Anthem.Manager).Assembly.GetManifestResourceStream("Wis.Toolkit.WebControls.Anthem.Anthem.js");
                StreamReader sr = new StreamReader(stream);
                page.ClientScript.RegisterClientScriptBlock(this.GetType(), "Anthem.js",
                    @"<script type=""text/javascript"">
//<![CDATA[
