//------------------------------------------------------------------------------
// <copyright file="PopupAnchor.cs" company="Oriental Everwisdom">
...
namespace Wis.Toolkit.WebControls
{
  ...
  /// </summary>
  [DefaultProperty("PopupToShow"), ToolboxData("<{0}:PopupAnchor runat=server></{0}:PopupAnchor>")]
  [Designer(typeof(AnchorDesigner))]
  public class PopupAnchor : System.Web.UI.WebControls.WebControl
  {
    #region Variables
    ...
    /// <summary>
    /// Should texts on Popup be replaced with new texts ?
    /// </summary>
    [Bindable(true),Category("Popup"),DefaultValue(false)]
    [Description("Should texts on Popup be replaced with new texts ?")]
      ...
      if (bChangeText)
      {
        ...
        if (ct!=null)
        {
          output.Write(controlId+"nText=\""+
            ((Popup)ct).GetWinText(snTitle,snText)+"\";\n");
        }
  ...
  /// <summary>
  /// Class for displaying Popup in designer
  /// </summary>
  public class AnchorDesigner : ControlDesigner
  {
    ...
    public override string GetDesignTimeHtml()
    {
      return "<div style=\"padding:2px; background-color: ButtonFace;color:ButtonText; "+
        ...
        "PopupAnchor</b> - "+((Control)Component).ID+"</div>";
    }
  ...
  /// <summary>
  /// Editor for selecting Popup controls
  /// </summary>
  public class PopupControlsEditor : ControlsEditor
  {
    ...
    /// Invoke base constructor
    /// </summary>
    public PopupControlsEditor() : base(typeof(Popup)) {}
