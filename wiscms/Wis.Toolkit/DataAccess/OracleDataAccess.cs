//------------------------------------------------------------------------------
// <copyright file="OracleDataAccess.cs" company="Oriental Everwisdom">
...
namespace Wis.Toolkit.DataAccess
{
	...
	public sealed class OracleDataAccess : AbstractDataAccess
	{
		...
		public OracleDataAccess(OracleConnection connection)
		{
		...
		private OracleConnection _DbConnection = new OracleConnection();
		...
		private void PrepareCommand(OracleCommand command, CommandType commandType, string commandText, IDataParameter[] commandParameterCollection)
		{
				...
				for (int index = 0; index < commandParameterCollection.Length; index++)
				{
					...
					switch (parameter.Value.GetType().ToString())
					{
						...
						case "System.Boolean": //bit 类型的字段如何处理？
							//if(oracleParameters[index].Value == null)
							//{
							//	oracleParameters[index].Value = DBNull.Value;
							//}
							break;
						case "System.String":
